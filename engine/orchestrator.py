...
from engine.llm import LLMAgent
...
async def run_quiz_flow(payload: dict):
    ...
    deadline = datetime.utcnow() + timedelta(seconds=175)
    quiz_summary = []
    ...
    while datetime.utcnow() < deadline and current_url:
        ...
        # --- LLM extraction ---
        llm_json = await llm_agent.get_structured_response(
            page_text,
        ...
        next_url = clean_url(llm_json.get("next_url"))
        submit_url = clean_url(llm_json.get("submit_url", "https://tds-llm-analysis.s-anand.net/submit"))
        ...
        if python_code:
            try:
                ...
                answer = None # set answer to None if code fails
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                resp_json = {"correct": False, "reason": "Submission failed"}
        ...
        if resp_json.get("url"):
            current_url = clean_url(resp_json.get("url"))
        elif next_url:
